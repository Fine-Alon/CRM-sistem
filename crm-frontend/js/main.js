...
const SERVER_URI = 'http://localhost:3000/api/clients',
    ...
    svgPhone = `<svg class="svgPhone" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g opacity="0.7">
...
function $createClientHTML(instanceClient) {
    ...
    for (const contact of contacts) {
        ...
        switch (contact.type) {
            case 'tel': $clientContactsItem.innerHTML = svgPhone
                break;
    ...
    $clientOptionsChange.addEventListener('click', async () => {
        ...
        if (dataToBeChange.contacts[0]) {
            ...
            for (let contactObj = 0; contactObj < dataToBeChange.contacts.length; contactObj++) {
                addContactBtnChangeField.click()
...
addContactBtn.addEventListener('click', () => {
    ...
    addClientContactID += 1
    contactID += 1
    if (addClientContactID > 3) { addContactBtn.style.display = 'none' }

    // check if exist any contact field so add display = 'flex' to its wrapper
    const countOfContaktFields = formContactWrapper.querySelectorAll('.form__contact')
    if (countOfContaktFields.length > 0) {
        formContactWrapper.style.display = 'flex'
    } else {
        formContactWrapper.style.display = 'none'
...
// add contact BTN usability 'ChangeField'
addContactBtnChangeField.addEventListener('click', () => {

    const addContactChangeField = $formContactDOM()

    formContactWrapperChangeField.append(addContactChangeField)
    changeClientContactID += 1
    contactID += 1

    if (changeClientContactID > 3) { addContactBtnChangeField.style.display = 'none' }

    // check if exist any contact field so add display = 'flex' to its wrapper
    const countOfContaktFields = formContactWrapperChangeField.querySelectorAll('.form__contact')
    if (countOfContaktFields.length > 0) {
        formContactWrapperChangeField.style.display = 'flex'
    } else {
        formContactWrapperChangeField.style.display = 'none'
